...
#pragma acc data create(a,b,c,bnd,wrk1,wrk2) copyout(p)
{
  initmt();
...
#pragma acc data present(a,b,c,p,bnd,wrk1)
  {
...
#pragma acc data present(a,b,c,p,wrk1,wrk2,bnd)
{
  for(n=0 ; n<nn ; ++n){
    gosa = 0.0;
...
#pragma acc kernels loop gang
    for(i=1 ; i<imax-1 ; i++)
      for(j=1 ; j<jmax-1 ; j++)
#pragma acc loop reduction(+:gosa) vector(512)
        for(k=1 ; k<kmax-1 ; k++){
